<?php
class qodyOverseer_FBMeta extends QodyOverseer
{
	...
	function CodeInsertion()
	{
		...
		{		
			...
			if( $custom['use_custom_title'] == 1 ) $content_title = $custom['custom_title'];
			else $content_title = $post->post_title;
			
			if( $custom['use_content_url'] == 1 ) $content_url = $custom['content_url'];
			else $content_url = get_permalink( $post->ID );
	
			$nextItem = 'content_type';
			if( $custom[ $nextItem ] ) $content_type = $custom[ $nextItem ];
			else $content_type = $this->get_option( $nextItem );
			
			$nextItem = 'custom_preview_image';
			switch( $custom[ $nextItem ] )
			{
				case '1':
				...
				case '2':
					$preview_image = $custom['preview_image_from_url'];
					break;
					
				case '3':
					$imageData = wp_get_attachment_image_src( $custom["qody_".$post->post_type."_1_thumbnail_id"] );
			...
			if( $custom['use_description'] == 1 )
			{
				$content_description = $post->post_excerpt;
			}
			else if( $custom['use_description'] == 2 )
			{
				$content_description = $custom[ $nextItem ];
			}
			...
			if( $custom['use_video'] == 1 )
			{
				$video_url = $custom['video_url'];
				$video_width = $custom['video_fb_width'];
				$video_height = $custom['video_fb_height'];
				$video_type = $custom['video_type'];
			}
			
			if( $custom['use_audio'] == 1 )
			{
				$audio_url = $custom['audio_url'];
				$audio_title = $custom['audio_title'];
				$audio_artist = $custom['audio_artist'];
				$audio_album = $custom['audio_album'];
				$audio_type = $custom['audio_type'];
			}
		}
